# -*- coding: utf-8 -*-
"""
Created on Tue May 16 21:12:35 2023
...
for i in range(0,30):
    ff[i], mean_ff[i], range_ff[i] = extract_frequency_related_information(r'C:\Users\manan\Documents\Predict_Confidence_Level_of_Speaker-main\Predict_Confidence_Level_of_Speaker-main\Common-Voice-Files-In-WAV-Format\common_voice_en ({}).wav'.format(i+1))
    amplitude_mean[i], amplitude_variance[i] = extract_amplitude_related_information(r'C:\Users\manan\Documents\Predict_Confidence_Level_of_Speaker-main\Predict_Confidence_Level_of_Speaker-main\Common-Voice-Files-In-WAV-Format\common_voice_en ({}).wav'.format(i+1))

#for i in range(0, 30):
#    filename = 'common_voice_en ({}).wav'.format(i+1)
#    speech_rate[i], pause_frequency[i], balance[i], articulation_rate[i] = analyze_audio_file(filename, temp_filename = '{}_temp.wav'.format(filename[:-4]), path = r'C:\Users\manan\Documents\Confidence Information Speech Task - Interview\Common-Voice-Files-In-WAV-Format')
    
# Initialize Input Array of Independent Features

x = [[0 for i in range(5)] for j in range(30)] 
...
model.save('Confidence_Scores_Model_Alternate.hdf5', overwrite=True, include_optimizer=True)

# Make a Sample Prediction through the model

model = load_model('Confidence_Scores_Model_Alternate.hdf5')

xp = [[250, 350, 400, 0.05, 0.006]]
